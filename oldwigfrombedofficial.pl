...
./introninfo inputintron.bed annotation.bed genome.2bit outputfile.tsv conservationtrack
...
conservationfile (wig format , optional)

You will need twoBitToFa in your path

(and hgWiggle if you are using conservation)
...
$intronbed=$ARGV[0];
$annotationbed=$ARGV[1];
$genome = $ARGV[2];
$outfile = $ARGV[3];
if($ARGV[4]){$conswig = $ARGV[4];}
else{$conswig="";}

if($intronbed =~ /[^$safe_filename_characters]/){print "unsafe sequence characters in intronbed \n"; print "please use simple alpha numeric characters\n";exit;}
...
%cnthash = (); %jutotalhash = ();
open(OF,">$outfile");
print OF "Intron_name\tlink_to_browser\tchrom\tstart\tend\tstrand\tlength\t5pss_to_bp\tbp_to3pss\ttype\tus\tbp_seq\tds\texpanded 5pss_seq\t5pss_conservation\texpanded_branch_point\tbp conservation\texpanded 3pss\t3pss conservation\ttotal_score\tgene\tannotated_intron_overlap\tintron_sequence\n";
open(IF,$intronbed);
while($a=<IF>){
...
$l=$start;$r=$end;
system("twoBitToFa /gbdb/sacCer3/sacCer3.2bit:$chrom:$l-$r tmp.fa");
open(FA1 , "tmp.fa");
$fl=<FA1>;
...
if($conswig ne ''){
#system("hgWiggle -db=sacCer3 -position=$chrom:$usl-$usr -doStats phastCons7way > tmp.stats");
...
#system("hgWiggle -db=sacCer3 -position=$chrom:$dsl-$dsr -doStats phastCons7way > tmp.stats");
...
if($conswig ne ''){
#system("hgWiggle -db=sacCer3 -position=$chrom:$l-$r -doStats phastCons7way > tmp.stats");
...
$gene =~ s/,$//;
$notes =~ s/,$//;
print  OF "$juname{$jupos}\t<a href=\"http://genome.ucsc.edu/cgi-bin/hgTracks?db=sacCer3&position=$chrom:$l-$r\">link</a>\t$chrom\t$start\t$end\t$strand\t$length\t$opos\t$actaapos\t$motif\t$us\t$actualbranch\t$ds\t$exus\t$meanus\t$exbranch\t$meanbranch\t$exds\t$meands\t$jutotalhash{$jupos}\t$gene\t$notes\t$full\n";
