...
@files = qw(
bayanus_rapa00_ls079_sacBay2
bayanus_rapa60_ls080_sacBay2
);
...
elsif($b[1]<$end && $b[2]>$start && $strand ne $b[5] && $notes !~ /opposing_intron_$b[3],/ ){ $notes .= "opposing_intron_$b[3],";$ihit++;}
}
}
...
system("hgWiggle -db=sacBay2 -position=$chrom:$usl-$usr -doStats $conswig > tmp.stats ");
open(ST,"tmp.stats");
$aa=<ST>;$aa=<ST>;
$aa=<ST>;$aa=<ST>;
if($aa){@bb= split(/\t/,$aa);
...
system("hgWiggle -db=sacBay2 -position=$chrom:$dsl-$dsr -doStats $conswig > tmp.stats");
open(ST,"tmp.stats");
$aa=<ST>;$aa=<ST>;
$aa=<ST>;$aa=<ST>;
if($aa){@bb= split(/\t/,$aa);
...
if($conswig ne ''){
system("hgWiggle -db=sacBay2 -position=$chrom:$l-$r -doStats $conswig > tmp.stats");
open(ST,"tmp.stats");
$aa=<ST>;$aa=<ST>;
$aa=<ST>;$aa=<ST>;
if($aa){@bb= split(/\t/,$aa);
...
$gene =~ s/,$//;
$notes =~ s/,$//;
print  OF "$juname{$jupos}\t<a href=\"http://intron.ucsc.edu/cgi-bin/hgTracks?db=sacBay2&position=$chrom:$l-$r\">link</a>\t$chrom\t$start\t$end\t$strand\t$length\t$opos\t$actaapos\t$motif\t$us\t$actualbranch\t$ds\t$exus\t$meanus\t$exbranch\t$meanbranch\t$exds\t$meands\t$gene\t$notes\t$jutotalhash{$jupos}";
