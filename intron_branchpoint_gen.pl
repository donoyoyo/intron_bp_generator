...
system("bigWigSummary $conswig $chrom $usl $usr 1 > tmp.stats ");
open(ST,"tmp.stats");
$aa=<ST>;
chomp($aa);
if($aa){$meanus = $aa;}else{$meanus=0;}
...
system("bigWigSummary $conswig $chrom $dsl $dsr 1 > tmp.stats ");
open(ST,"tmp.stats");
$aa=<ST>;
chomp($aa);
if($aa){$meands = $aa;}else{$meands=0;}
}
...
$aa=<ST>;
chomp($aa);
if($aa){$meanbranch = $aa;}else{$meanbranch=0;}
}
else{$meanbranch=0;}
}
